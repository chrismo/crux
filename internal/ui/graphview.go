...
// this v1 conveys structure via layering and state via color/glyph.
func RenderGraph(g *graph.Graph, l *graph.LayoutData) string {
	rows := make([]string, 0, len(l.Layers))
	for _, layer := range l.Layers {
		cells := make([]string, 0, len(layer))
		for _, key := range layer {
			cells = append(cells, renderCell(g.Node(key)))
		}
...
func renderCell(n *graph.Node) string {
